...
URL insert_url_list(URL head, URL new)
{
    ...
    // URLs that have the highest number of terms will appear first. Thus, if
    // new has more terms than the head, prepend new to the linked list and return new.
    if (head->terms < new->terms)
    {
        new->next = head;
        return new;
    }
    ...
    // Iterate through the list and search for when the URLs either have equal to 
    // or less terms than new
    while (curr->next != NULL)
    {

        // If the term count is equal between curr and new, continue iterating while
        // the terms are ranked less in terms of lexical order
        if (curr->next->terms == new->terms)
        {
            URL cursor = curr;
            while ((cursor->next != NULL) && (cursor->next->terms == new->terms))
            {
                if (strcmp(new->url, cursor->next->url) < 0)
                {
            ...
            // Number of terms in the list is no longer equal to new->terms
            else if (cursor->next->terms < new->terms)
            {
                new->next = cursor->next;
                cursor->next = new;
            }
        ...
        // Found the point to insert new such that the URLs are in descending order of
        // term count
        if (curr->next->terms < new->terms)
        {
            new->next = curr->next;
...
void descending_print_tfidf(RBTree tree, int limit, int *count)
{
    ...
    else
    {
        ...
        {
            printf("%s %.6f %d\n", curr->url, tree->tfidf, curr->terms);
            ++(*count);
            curr = curr->next;
        }
    }
...
int main(void)
{
    ...
    infix_tfidf_print(new->root); printf("\n\n\n");
    Tree_Rep fake = new_RBTree();
    RBTree_insert_url(fake, 50, "replica");
    RBTree_insert_url(fake, 50, "is");
    RBTree search = RBTree_search_url(fake->root, "replica");
    search->url->terms = 100;
 
    transfer_nodes(fake->root, new); 
    ...
    *count = 0;
    descending_print_group(new->root, 30, count);
    printf("%d NUM OF VALUES\n", *count);
    printf("\n\n\n");
    free(count);
    free_RBTree(fake);
    free_RBTree(new);
